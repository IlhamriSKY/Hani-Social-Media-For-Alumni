...
class bukualumni extends db_connect
{
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM buku_alumni WHERE removeAt = 0");
        $stmt->execute();
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM buku_alumni WHERE removeAt = 0 AND fromUserId = (:fromUserId)");
    ...
    private function getCount($queryText)
    {
        ...
        $sql = "SELECT count(*) FROM buku_alumni WHERE removeAt = 0 AND (itemTitle LIKE '{$queryText}' OR itemDesc LIKE '{$queryText}' OR itemContent LIKE '{$queryText}')";
    ...
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM buku_alumni");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn() + 1;
    }

    public function add($title, $content, $imgUrl, $previewImgUrl, $allowComments = 1, $postArea = "", $postCountry = "", $postCity = "", $postLat = "0.000000", $postLng = "0.000000", $shareto)
    {
        $result = array(
            "error" => true,
        ...
        $nim = $profileInfo['nim'];
        $nama = $profileInfo['fullname'];

        $title = $nim.' - '.$nama.' - '.$title;

        $stmt = $this->db->prepare("INSERT INTO buku_alumni (allowComments, fromUserId, itemTitle, itemDesc, itemContent, imgUrl, previewImgUrl, area, country, city, lat, lng, createAt, ip_addr, u_agent, shareto) value (:allowComments, :fromUserId, :itemTitle, :itemDesc, :itemContent, :imgUrl, :previewImgUrl, :area, :country, :city, :lat, :lng, :createAt, :ip_addr, :u_agent, :shareto)");
        ...
        $stmt->bindParam(":shareto", $shareto, PDO::PARAM_INT);
    ...
    public function remove($itemId)
    {
        ...
        $stmt = $this->db->prepare("UPDATE buku_alumni SET removeAt = (:removeAt) WHERE id = (:itemId)");
    ...
    public function query($queryText = '', $itemId = 0)
    {
        ...
        $sql = "SELECT * FROM buku_alumni WHERE removeAt = 0 AND (itemTitle LIKE '{$queryText}' OR itemDesc LIKE '{$queryText}' OR itemContent LIKE '{$queryText}') AND id < {$itemId}".$endSql;
        $stmt = $this->db->prepare($sql);
    ...
    public function preload($itemId = 0)
    {
        ...
        $sql = "SELECT * FROM buku_alumni WHERE removeAt = 0 AND id < {$itemId}".$endSql;
        $stmt = $this->db->prepare($sql);
    ...
    public function getMyItems($itemId = 0)
    {
        ...
        $stmt = $this->db->prepare("SELECT * FROM buku_alumni WHERE removeAt = 0 AND id < (:itemId) AND fromUserId = (:fromUserId) ORDER BY id DESC LIMIT 20");
        $stmt->bindParam(':fromUserId', $this->requestFrom, PDO::PARAM_INT);
        $stmt->bindParam(':itemId', $itemId, PDO::PARAM_INT);
    ...
    public function getItem($itemId = 0)
    {
        ...
        $stmt = $this->db->prepare("SELECT * FROM buku_alumni WHERE removeAt = 0 AND id = (:itemId)");
        $stmt->bindParam(':itemId', $itemId, PDO::PARAM_INT);
